...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  float t = iTime * 0.3; // Slow down animation
  ...
  speedAdjustment = zoomSpeed * cos(t * 0.02 + 3.1415926 / 4.0);          

  vec2 uvCoordinates = scaledCoordinates;		       

  float rotationXZ = 0.9;
  float rotationYZ = -0.6;
  float rotationXY = 0.9 + t * 0.08;	
  ...
  cameraPosition.x += transverseMotion * cos(0.01 * t) + 0.001 * t;
  cameraPosition.y += transverseMotion * sin(0.01 * t) + 0.001 * t;
  cameraPosition.z += 0.003 * t;	
  ...
  backgroundColor.bg = mix(backgroundColor.gb, backgroundColor.bg, 0.5 * (cos(t * 0.01) + 1.0));	
